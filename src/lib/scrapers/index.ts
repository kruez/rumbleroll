...
import { scrapeSmackdownHotel } from "./smackdownhotel-scraper";
import { ScrapeResult } from "./types";
import { prisma } from "@/lib/prisma";

export type { ScrapedWrestler, ScrapeResult } from "./types";
export { scrapeSmackdownHotel } from "./smackdownhotel-scraper";
...
/**
 * Run the SmackdownHotel scraper and save results to the database
 * This source provides 300+ wrestlers without JS pagination
 */
export async function runSmackdownHotelScrape(force = false): Promise<ScrapeResult> {
  // Check if we should skip (scraped recently)
  if (!force) {
    const metadata = await prisma.scrapeCacheMetadata.findUnique({
  ...
  // Run the scraper
  const result = await scrapeSmackdownHotel();
    ...
    try {
      await prisma.wrestler.upsert({
        ...
        create: {
          ...
          brand: wrestler.brand,
          source: wrestler.source,
          sourceId: wrestler.slug, // Use slug as sourceId for SmackdownHotel
          isActive: true,
          aliases: [],
        },
        update: {
